...
- Add time scales, each converting to and from TAI, the scale an instant is
  stored in: `TAI`, `TT` at its fixed 32.184 s, `TDB` over the full
  787-term Fairhead and Bretagnon model, and `UTC` with its leap seconds
...
- Add the representations a reading comes out as: `julian_date`,
  `modified_julian_date`, `civil`, and `iso8601`, each as a Float, a
  Rational, or a two-part Float where that makes sense
...
- Add `Configuration#register_scale`, so a caller can add a scale of its own,
  and `Configuration#leap_second_source`, so it can supply its own leap
  seconds
- Add duration arithmetic: `+`, `-`, unary `-`, `zero?`, `negative?`,
  `positive?`, and `to_r` and `to_f` in SI seconds
- Add value equality to `ScaleReading`, and `inspect` to `Instant`,
  `Duration`, and `ScaleReading`
...
**Full Changelog**: https://github.com/rhannequin/horologium/compare/v0.0.2...HEAD
